...
export default function Header() {
    ...
    const [cotizacion, setCotizacion] = useState<number>(1150);
    ...
    useEffect(() => {
        ...
        async function updateRate() {
            ...
            if (rate) {
                setCotizacion(rate);
                localStorage.setItem("last_dollar_rate", rate.toString());
            } else {
                const lastRate = localStorage.getItem("last_dollar_rate");
                if (lastRate) setCotizacion(parseFloat(lastRate));
            }
        }
        updateRate();
        ...
        window.addEventListener("storage", () => { checkUser(); updateCart(); });
            ...
            <div className="w-full px-4 md:px-12 lg:px-20 flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3 lg:gap-6 shrink-0">
                    <div className={`hidden sm:flex flex-col items-end border-r pr-6 ${scrolled ? "border-white/20" : "border-gray-200"}`}>
                        <span className={`text-[10px] uppercase font-bold tracking-widest ${scrolled ? "text-white/60" : "text-zinc-500"}`}>Dólar Oficial</span>
                    ...
                    <div className="relative">
                        ...
                        {isCartOpen && (
                            <div className="absolute right-0 mt-3 w-80 bg-white rounded-xl shadow-xl ring-1 ring-black ring-opacity-5 z-50 overflow-hidden text-gray-900">
                                <div className="p-4">
                                    <h3 className="font-bold border-b pb-2 mb-3 text-sm">Tu Pedido</h3>
                                    {cartItems.length === 0 ? (
                                        ...
                                        <div className="max-h-60 overflow-y-auto space-y-3">
                                            {cartItems.map((item) => (
                                                <div key={item.id} className="flex justify-between items-start text-sm">
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        <span className="font-semibold">${(item.precio * item.quantity).toLocaleString()}</span>
                                            ...
                                            <div className="border-t border-gray-100 mt-3 pt-3">
                                                <div className="flex justify-between items-center mb-1">
                                                    ...
                                                    <span className="text-lg font-bold text-[#1F4E79]">${totalAmount.toLocaleString()}</span>
                                                </div>
                                                <CheckoutAction />
                                            </div>
                                        </div>
                                ...
                                {isProfileOpen && (
                                    <div className="absolute right-0 mt-3 w-64 bg-white rounded-xl shadow-2xl ring-1 ring-black/5 z-50 overflow-hidden transform origin-top-right transition-all animate-in fade-in slide-in-from-top-1 text-gray-700">
                                        <div className="px-5 py-4 bg-gray-50 border-b border-gray-100">
                                            ...
                                            {usuario.rol === "ADMIN" && (
                                                <span className="inline-block mt-1 px-2 py-0.5 bg-amber-100 text-amber-700 text-[10px] font-black rounded uppercase">
                                                    Administrador
                                                </span>
                                            )}
                                        </div>
                                        <div className="p-2 flex flex-col gap-1">
                                            {usuario.rol === "ADMIN" && (
                                                <Link href="/admin/pedidos" onClick={() => setIsProfileOpen(false)} className="group flex items-center w-full px-4 py-3 text-sm font-bold rounded-lg bg-amber-50 text-amber-700 hover:bg-amber-100 transition-all gap-3 mb-1">
                                                    <div className="flex items-center justify-center w-8 h-8 rounded-full bg-white text-amber-600 group-hover:scale-110 transition-transform shrink-0 shadow-sm border border-amber-100">
